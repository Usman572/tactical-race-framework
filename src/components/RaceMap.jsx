...
export default function RaceMap({ races = [] }) {
    ...
    useEffect(() => {
        ...
        if (socket) {
            socket.on('territory_update', (updatedTerritory) => {
                setTerritories(prev => prev.map(t => 
                    t._id === updatedTerritory._id ? updatedTerritory : t
                ));
            });

            socket.on('territory_flip', (data) => {
                console.log(`Sector ${data.sectorName} claimed by ${data.newOwner}!`);
                fetchTerritories();
            });
                    ...
                    return (
                        <Polygon
                            ...
                            pathOptions={{
                                ...
                                fillOpacity: isFocused ? (0.1 + (influencePercent / 1000)) : 0.05,
                                weight: isFocused ? 3 : 1,
                                dashArray: isFocused ? '10, 10' : '0',
                                lineJoin: 'round',
                                className: `territory-polygon ${isFocused ? 'animate-pulse-slow' : ''}`
                            }}
                        >
                            <Popup>
                                <div className="text-[10px] font-black uppercase tracking-widest text-slate-900 min-w-[150px]">
                                    <div className="mb-2 border-b border-slate-200 pb-1 flex justify-between items-center">
                                        <span>{territory.sectorName}</span>
                                        <span className="text-[8px] bg-slate-100 px-1 rounded">SEC_ID: {territory._id.slice(-4)}</span>
                                    </div>
                                    <div className="space-y-1">
                                        <div style={{ color: ownerColor }}>Owner: {territory.currentOwner}</div>
                                        <div className="flex flex-col gap-1 mt-2">
                                            <div className="flex justify-between text-[8px]">
                                                <span>Influence Control</span>
                                                <span>{Math.round(influencePercent)}%</span>
                                            </div>
                                            <div className="w-full h-1 bg-slate-200 rounded-full overflow-hidden">
                                                <div 
                                                    className="h-full transition-all duration-1000" 
                                                    style={{ width: `${influencePercent}%`, backgroundColor: ownerColor }}
                                                />
                                            </div>
                                        </div>
                                    </div>
