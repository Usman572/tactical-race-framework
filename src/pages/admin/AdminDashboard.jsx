    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.3em] italic opacity-60">Sector Synchronization & Telemetry</p>
        </div>
        ...
        <div className="flex items-center gap-4 bg-[var(--header-bg)] backdrop-blur-md p-5 rounded-2xl border border-[var(--border-main)] shadow-xl relative overflow-hidden group">
          ...
          <div className="w-12 h-12 rounded-xl bg-blue-600 text-white flex items-center justify-center text-xl shadow-glow-primary relative z-10 transition-transform group-hover:scale-110">🛡️</div>
          <div className="relative z-10">
            <p className="text-[9px] font-black text-blue-500 uppercase tracking-[0.3em]">Authorized Access</p>
            <p className="text-sm font-black text-[var(--text-main)] italic">Operative: <span className="text-red-500 uppercase">{currentUser?.name || 'ADMIN'}</span></p>
          </div>
        </div>
      </div>
      ...
      >
        {[
          { label: "Active Agents", value: stats?.users?.total || 0, color: "from-blue-600 to-indigo-700", accent: "var(--accent-primary-glow)", icon: "👥" },
          { label: "Strategic Partners", value: stats?.users?.partners || 0, color: "from-purple-600 to-fuchsia-700", accent: "rgba(147, 51, 234, 0.2)", icon: "🤝" },
          { label: "Total Engagements", value: stats?.races?.total || 0, color: "from-emerald-500 to-teal-700", accent: "rgba(16, 185, 129, 0.2)", icon: "🏁" },
          { label: "Operational Load", value: stats?.system?.load || "...", color: "from-orange-500 to-red-700", accent: "var(--accent-secondary-glow)", icon: "⚡" }
        ].map((stat, index) => (
          <motion.div
            key={index}
            variants={{
          ...
          >
            ...
            <div className="p-8">
              ...
              <div className="space-y-1">
                ...
                <p className="text-5xl font-black text-[var(--text-main)] tracking-tighter italic">{stat.value}</p>
              </div>
